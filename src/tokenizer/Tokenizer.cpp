    ...
    while(!input.eof()) {
      ...
      if(isPunctuation(theChar)) {
        ...
        if(theToken.data.find(',')!=std::string::npos){
            theToken.data.erase(theToken.data.find(','),1);
            tokens.push_back(theToken);
            Token theToken2{TokenType::punctuation, Keywords::unknown_kw, Operators::unknown_op};
            theToken2.data=(",");
            tokens.push_back(theToken2);
        }
        else if(theToken.data.find(')')!=std::string::npos){
            theToken.data.erase(theToken.data.find(')'),1);
            tokens.push_back(theToken);
            Token theToken2{TokenType::punctuation, Keywords::unknown_kw, Operators::unknown_op};
            theToken2.data=(")");
